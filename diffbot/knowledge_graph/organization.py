...
def _parseCategories(data: dict, 
    target='Packaging Companies', _min: int=10):
...
def _parseClassificationCodes(data: dict, class_abbrv: str, 
                              target: str="plastic", _min: int=3):
    ...
    try:
        ...
        if np.any(targets):
            [idx.append(i) for i, j in enumerate(targets) 
                if j==True and i not in idx]
        ...
        clfCodes = {k: codes[v] if v is not None else None 
            for k, v in zip(keysCode, idx)}
        clfDesc = {k: desc[v] if v is not None else None 
            for k, v in zip(keysDesc, idx)}
...
def _parseNAICSClassification(data: dict, class_abbrv: str='naics', 
                              target: str='plastic', _min: int=3):
    return _parseClassificationCodes(data, class_abbrv, target, _min)

def _parseSICClassification(data: dict, class_abbrv: str='sic', 
                            target: str='plastic', _min: int=3):
    return _parseClassificationCodes(data, class_abbrv, target, _min)

def _parseISICClassification(data: dict, class_abbrv: str='isic', 
                              target: str='plastic', _min: int=3):
    return _parseClassificationCodes(data, class_abbrv, target, _min)

def _parseNACEClassification(data: dict, class_abbrv: str='nace', 
                              target: str='plastic', _min: int=3):
    return _parseClassificationCodes(data, class_abbrv, target, _min)
